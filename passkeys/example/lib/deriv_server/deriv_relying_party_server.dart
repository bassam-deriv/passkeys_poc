...
import 'package:passkeys_example/deriv_server/request.dart';
import 'package:passkeys_example/deriv_server/response.dart';


/// Implementation of [RelyingPartyServer] that allows to use Corbado as a
/// relying party.
class DerivPasskeyBackend
    extends RelyingPartyServer<AuthRequest, AuthResponse> {
  /// Sets up the client for the Corbado API.
  DerivPasskeyBackend(this._projectID)
      :
  //       _client =
  ...
  Future<RegistrationInitResponse> initRegister(AuthRequest request) async {
    ...
    return RegistrationInitResponse(
      RelyingParty("test", "id"),
      User("DisplayName","name","id"),
      "asdfasdf",
      AuthenticatorSelection("",true,"",""),
      timeout: 10,
      attestation: "publicKey.attestation",
      pubKeyCredParams: ["publicKey.pubKeyCredParams"]
          .map((e) => PubKeyCredParam(1, "e.type"))
          .toList(),
    );
  ...
  @override
  Future<AuthResponse> completeAuthenticate(AuthenticationCompleteRequest request) {
    // TODO: implement completeAuthenticate
    throw UnimplementedError();
  }

  @override
  Future<AuthenticationInitResponse> initAuthenticate(AuthRequest request) {
    // TODO: implement initAuthenticate
    throw UnimplementedError();
  }
